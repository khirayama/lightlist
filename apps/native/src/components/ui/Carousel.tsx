...
export function Carousel({
  ...
  const lastDirectionRef = useRef(uiDirection);
  ...
  const pagerKey = `${uiDirection}-${count}`;
  ...
  useEffect(() => {
    ...
    lastDirectionRef.current = uiDirection;
  }, [count, uiDirection]);

  useEffect(() => {
    if (count === 0) return;
    if (lastDirectionRef.current === uiDirection) return;
    lastDirectionRef.current = uiDirection;
    pagerIndexRef.current = currentIndex;
    hasControlledSyncRef.current = false;
    lastEmittedIndexRef.current = currentIndex;
    requestAnimationFrame(() => {
      pagerRef.current?.setPageWithoutAnimation(currentIndex);
    });
  }, [count, currentIndex, uiDirection]);
