...
- [4. 内部アーキテクチャ（Loro統合とCollaborative機能）](#4-内部アーキテクチャloro統合とcollaborative機能)
...
本SDKは、タスク管理アプリのクライアントとAPI間の通信、リアルタイム共同編集、状態管理を統合的に提供します。
...
Actions Layer (Business Logic)
    ↓ ↓
Services Layer (API Communication)    Store Layer (State Management + Loro)
```
...
- Store Layer: 状態管理と Loro。Actions Layer からのみ更新される
...
- CollaborativeService: Loro共同編集とセッション管理
...
- TaskListActions: タスクリスト操作（Loro協調編集を内部で自動実行）
- TaskActions: タスク操作（リアルタイム同期を内部で自動実行）
- ShareActions: タスクリスト共有機能

重要: すべてのタスク・タスクリスト操作はLoro経由でリアルタイム同期が自動実行されます。同期処理を手動で管理する必要はありません。
...
- Loro協調編集とリアルタイム同期を内部で自動実行

## 4. 内部アーキテクチャ（Loro統合とCollaborative機能）
...
1. Loro統合エンジン: ドキュメント作成・同期・競合解決を自動実行
2. 協調編集マネージャー: セッション管理と自動競合解決
3. 内部データ構造: LoroのCRDTでタスク順序を管理、内容はLast-Writer-Wins
4. 自動競合解決: タスク順序は自動マージ、同時編集は最終更新優先

### ユーザー操作と内部処理の対応

```
タスクリスト作成 → Loroドキュメント作成 + セッション開始 + リアルタイム通知
タスク作成      → 楽観的更新 + Loro追加 + リアルタイム同期 + サーバーバックアップ
...
2. Loro統合: ドキュメント管理とリアルタイム同期
3. 購読管理: UI コンポーネントへの状態変更通知
4. セレクター: 効率的な状態の計算と取得

### 主要コンポーネント

- Store: メインストア（状態、Loro、リスナー管理）
...
- ConflictResolver: Loro競合の自動・手動解決
...
重要: 内部依存（協調編集マネージャー、Loro統合）はユーザーには公開されません。
