...
- アプリアイコン/スプラッシュ画像は `apps/native/assets`（`icon.png` / `adaptive-icon.png` / `splash-icon.png`）を正本とし、必要に応じて `ios/LightlistDev/Images.xcassets` と `android/app/src/main/res` の生成済み画像を同じロゴで更新する。現行アプリアイコン（`icon.png` / `adaptive-icon.png`）は既存デザインを踏襲し、ロゴレイヤーのみを従来比 `96%` に縮小して上下余白を微増した配置を採用する
...
- タスクリスト詳細はカルーセルで横スワイプ切り替えでき、スワイプ位置と選択中のリストIDを同期する。通常のスワイプ操作はカルーセルを優先し、`Carousel` は `index` / `onIndexChange` の Controlled 制御を前提とする。`scrollEnabled` は `TaskListCard` から受け取る `onSortingChange` と連動して、タスク並び替え中のみ横スワイプを無効化する。上部インジケーター（ドット）のタップでも切り替え可能
...
- `apps/native/app.config.ts`: Expo 設定。`plugins` に `expo-font` と `@react-native-community/datetimepicker` を設定する
