...
- 各タスクアイテムは以下の情報を含む：
  ...
  - 日付設定ボタン：タスクの日付を設定（未設定時は calendar_today アイコン、設定済みは `YYYY-MM-DD` を表示）
- 完了したタスクは取り消し線で表示される
...
1. タスクアイテムの日付設定ボタンをクリック（未設定時は calendar_today アイコン、設定済みは日付表示）
...
- **表示:** 未設定時は calendar_today アイコン、設定済みは `YYYY-MM-DD`（テキストは SR も付与）
