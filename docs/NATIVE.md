...
- `package.json`: `react` は Expo SDK 54 / Expo Go に合わせて 19.1.0 に固定し、`@types/react` は NativeWind 型互換を優先して 19.2.3 に固定する（`expo install --check` では型定義に対する互換警告が出る）。`react-native-screens` は Expo Go 互換で固定
