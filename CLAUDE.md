...
- カレンダー画面は淡いページ背景にグリッドを直置きし、日付付きタスク一覧のみボーダーレスの面カードに入れる。月ナビは枠なしアイコンボタン、日セルは選択 = 塗り円 / 当日 = 枠線円、日付ドットに縁取りなし。タスク行は「日付 + リストチップのメタ行 + 本文」の 2 行構成で、日付/本文タップ = 日付選択（絞り込みなし・ハイライト + スクロール）、チップタップ = タスクリストを開く、を 3 プラットフォームで統一（詳細は `AGENTS.md`）。
