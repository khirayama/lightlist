...
- 設定画面は Web の視覚階層を正とし、3 プラットフォームとも淡いページ背景の上に、境界線付きのカードを配置する。各セクション見出しはカード内の先頭に置き、設定行の操作領域は 44pt / 48dp 以上を維持する。
...
- パスワードリセット: `lightlist://password-reset?oobCode=...`（iOS / Android）、`https://lightlist.com/password_reset?oobCode=...`（Android）
