...
- Web / iOS / Android の task 更新 UI は、各操作ごとに直接 Firestore payload を組み立てず、`現在表示中 task 群 -> 正規化済み next task 群 -> pending 表示 -> queue 経由の差分保存` の順で処理する。正規化と pending 反映は操作ごとに一度だけ行い、同じ next task 群から保存差分を作る。pending 解放判定も listener 側 task 群を同じ正規化へ通した結果で比較する。Firestore へは新規 task の full object、削除 task の `tasks.<id>` delete、既存 task の変更 field と変化した `order` だけを書き込む。
