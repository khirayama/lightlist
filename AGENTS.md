...
- 3 プラットフォームのタスク行は追加約 240ms（fade、Web / iOS は move(top) も併用）、削除約 120ms（fade のみ）、並び替え約 220ms 相当の減衰した spring / ease-out で揃える。ドラッグ中の行は opacity `0.8` + scale `1.03`。Web はドラッグ中を dnd-kit `useSortable.transition`、autoSort 等のドラッグ外配置変更を `TaskItemComponent` の `offsetTop` と viewport top 差分 + Web Animations API（FLIP）で動かし、`ll-anim-task-enter` / `ll-anim-task-exit` も使う。FLIP は layout top が変化した場合だけ生成し、optimistic表示から同一配置のlistener表示へ切り替わる再描画では進行中Animationを cancel・再生成しない。iOS / Android の完了済み削除は `exitingTaskIds` で行 alpha を先に 0 へ動かし、約 120ms 後に pending task 群と保存差分から外す。iOS の通常配置変更は `setPendingTasks` の spring + 行 `.transition`、Android は `Modifier.animateItem(fadeInSpec / placementSpec / fadeOutSpec)` を使う。OS の reduce motion 設定（Web `prefers-reduced-motion` / iOS `accessibilityReduceMotion` / Android `reduceMotion`）時は無効化する。完了トグルの塗り scale + spring、行 alpha のフェード、ピン/カレンダーアイコンの切替、追加ボタンの表示切替も同設定を尊重し、iOS のアイコン切替は `contentTransition(.symbolEffect(.replace))`、Android は `Crossfade` を使う。
