...
- タスクリストの作成（名前＋色）、編集（名前＋色）、削除、ドロワー内の左端ハンドルアイコンで順序変更を提供
- 共有コードの生成/停止を提供
...
- **ドロワー:** 設定画面へのリンクとタスクリスト一覧をまとめ、選択時はドロワーを閉じてタスク操作に戻る。タスクリスト一覧は左端のハンドルアイコンで並び替えできる
...
ネイティブアプリでは共有コードを入力して読み込み、共有リストの閲覧・タスク追加・完了切り替え・削除・ドラッグ並び替え・日付入力・自分のリストへの追加に対応します。
