...
- `autoSort` 無効時は各グループ内の現在の `order` を維持し、設定切替だけで既存 task の順序を変更しない。画面下部の「並び替え」は明示操作として、設定値に関係なく日付順へ整列する。
- D&D はグループ内のみ。`autoSort` 有効時は `date` が同じ task 間だけ並び替え、`date` が異なる task の表示順は自動整列へ委ねる。グループ移動はピン切替・完了切替で行う。
...
- タスクシートの変更（追加先タスクリスト・本文・ピン留め・日付）は確定ボタンでまとめて保存する。日付クリアはシート内の選択状態だけを消し、確定時に `date` を空にする。
...
- listener 失敗は読み込めているデータを維持して部分劣化させ、同じ参照を 1 秒、2 秒、4 秒…最大 30 秒の間隔で再購読する。購読スコープが終了するまで再試行し、cache snapshot では再試行間隔をリセットしない。全画面エラーにするのは設定・保持リスト順序の失敗、または taskLists を 1 件も表示できない場合だけ。
