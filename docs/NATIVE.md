...
- `expo-splash-screen` を使用し、フォントロードと認証状態の確定が完了するまでスプラッシュスクリーンを表示し続けることで、起動時の画面のちらつきを防止。アイコンは SVG 化（`react-native-svg`）している
...
- タスク追加フォームは上部に配置し、入力欄にフォーカスしたときだけ send アイコンボタンをアニメーション表示して追加操作を行う。右側の calendar_today アイコンボタン（枠線なし、パディング調整）からDate Pickerで日付設定、完了切り替え、左端の drag_indicator アイコンボタン（枠線なし、パディング調整）による並び替え、ソート（sort アイコン付き、左寄せ、枠線なし、パディング調整）、完了タスク削除（delete アイコン付き、右寄せ、枠線なし、パディング調整）に対応
- タスクの並び替え操作は optimistic update を採用しており、ローカル状態を先に更新してからサーバーに保存する。サーバー保存に失敗した場合は元の状態にロールバックする。これにより、ドラッグ操作後のちらつきを防止している
