...
- **タスク詳細カルーセル:** 各タスクリストを `Carousel` (Embla) で横スライド化し、ホイール左右操作や前後ボタン、スワイプで切り替える。AppHeader 直下にドット型の locator を表示し、現在位置を示しつつクリックでリストを切り替えられる。表示中スライドの `TaskList.background` を外枠アクセントとして適用し、内側を可読性の高いサーフェス（ライト/ダーク対応）として構成する。`TaskListPanel` は共有ページと同じレイアウト（入力欄が上、一覧が下）で完了・日付設定・追加・編集を行う。
- **色と共有:** タスクリストカード右上の編集/共有アイコンボタンから Dialog を開き、編集Dialogでリスト名と背景色をまとめて変更する（保存はフォーム送信: `Enter` / 保存ボタン）。共有Dialogでコードの生成/停止とクリップボードコピーを行う。
...
- メニューアイコンは `app.openMenu` を `aria-label` / `title` に設定する。
...
**ページ:** `apps/native/App.tsx`

### 概要

- Web の app ページを参照し、タスクリスト一覧と選択中リストのタスクを同一画面で管理する構成
- タスクリストの作成（名前＋色）、編集（名前＋色）、削除、順序変更を提供
- 共有コードの生成/停止を提供
...
- **ヘッダー:** アプリタイトルとサインアウト操作を配置
- **タスクリスト一覧:** 横スクロールで選択、件数を表示
- **作成フォーム:** リスト名と背景色を入力して作成
- **編集フォーム:** リスト名と背景色を更新し、削除確認を行う
- **タスク:** 入力欄で追加し、一覧で編集（テキスト/期限）・完了切り替え・並び替え・削除が可能。ヘッダー操作でソートと完了タスク削除を行う
...
- 各タスクアイテムは以下の情報を含む：
  ...
  - 日付設定ボタン：タスクの日付を設定（未設定時はカレンダーアイコン、設定済みは `YYYY-MM-DD` を表示）
- 完了したタスクは取り消し線で表示される
...
1. タスクアイテムの日付設定ボタンをクリック（未設定時はカレンダーアイコン、設定済みは日付表示）
...
- **コーナー:** 丸角（`rounded-lg`）
- **内部構造:**
  - ドラッグハンドル、チェックボックス、テキスト、日付設定ボタンが左から右へ配置
...
- **表示:** 未設定時はカレンダーアイコン、設定済みは `YYYY-MM-DD`（テキストは SR も付与）
- **ホバー:** 薄いグレー背景（`hover:bg-gray-100` / `dark:hover:bg-gray-800`）
- **ツールチップ:** 設定済みの場合は「日付を設定: YYYY-MM-DD」を表示
...
**画面:** `apps/native/screens/ShareCodeScreen.tsx`

### 概要

shareCodeを使用して、認証なしでタスクリストを閲覧・編集できるページです。オーナーと同じUIでタスクの操作が可能であり、自分のリストに追加することもできます。

ネイティブアプリでは共有コードを入力して読み込み、共有リストの閲覧・タスク追加・完了切り替え・削除・自分のリストへの追加に対応します（ドラッグ並び替えや日付設定は行いません）。
...
- タスクの日付設定：日付設定ボタンから Date Picker を開く
