...
- カレンダーの日付選択時は、「選択日の表示 + タスクを追加」の主ボタンを表示し、「選択日 + 閉じる → 追加先タスクリスト → 本文 → 横幅いっぱいの追加ボタン」の sheet / dialog を開く。角丸・入力面・余白・文字階層は Web を正として揃え、選択日は固定し、日付付きタスクが0件でも追加可能にする。通常追加と同じ settings・履歴・taskList 単位 mutation queue を使い、listener 反映までは同じ taskId の楽観表示を維持する。
