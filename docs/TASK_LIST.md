...
- **タスクリスト一覧:** `appStore` から取得したリストを DnD で並び替え。作成ボタンは Dialog で開き、名前と背景色を入力して `createTaskList` を実行。リストが空のときは `app.emptyState` を表示。
- **タスク詳細:** 選択中リストのタスクを `TaskListPanel` に渡して表示。完了・削除・追加・編集をまとめて受け持ち、履歴候補 (`history`) を `datalist` で提示する。
- **色と共有:** 編集Dialogでリスト名と背景色をまとめて変更。共有Dialogでコードの生成/停止とクリップボードコピーを行う。
- **削除確認:** 削除専用Dialogで確認してから `deleteTaskList` を実行。
