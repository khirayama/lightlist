...
- アプリアイコン/スプラッシュ画像は `apps/native/assets`（`icon.png` / `adaptive-icon.png` / `splash-icon.png`）を正本とし、必要に応じて `ios/LightlistDev/Images.xcassets` と `android/app/src/main/res` の生成済み画像を同じロゴで更新する。現行アプリアイコン（`icon.png` / `adaptive-icon.png`）は既存デザインを踏襲し、ロゴレイヤーのみを従来比 `90%` に縮小して上下余白を微増した配置を採用する
