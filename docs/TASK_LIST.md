...
- ネイティブでは `Carousel`（`react-native-pager-view` ベース）の `scrollEnabled` を並び替え状態と連動させ、通常時は横スワイプを優先し、タスクアイテムの drag_indicator ハンドルからドラッグを開始した場合のみ横スワイプを無効化する。ハンドル起点のドラッグは長押し不要で開始できる。`Carousel` は `index` / `onIndexChange` の Controlled API を前提とし、index 確定は `onPageSelected` で行う。外部 index 変更はページャーへ直接反映して連続スワイプ時の不安定な戻りを防ぐ。
- リスト一覧での選択や Dialog オープン時は対象リストを事前に選択し、カルーセル位置とフォーム入力を一致させる。
- モバイルのドロワー内タスクリスト選択はドロワーを先に閉じ、`InteractionManager.runAfterInteractions` で選択反映を遅延実行する。これによりドロワー遷移中のページャー同期抜けを防ぎ、選択リストへのカルーセル移動を保証する。
...
3. Firestore でバッチ書き込みを実行：
   - `taskLists/{newId}` ドキュメントを作成（`memberCount: 1`）。
...
1. 「リストを一覧から外す」ボタンをクリック
2. 確認モーダルが表示される
3. 確認後、対象リストを自分の `taskListOrder` から削除する
4. 削除後の `memberCount` が 0 の場合のみ `taskLists/{taskListId}` 実体と `shareCodes/{shareCode}` を削除する
...
- 共有リスト購読の解除は shareCode 読み込み `useEffect` の cleanup で一元管理し、shareCode の切り替え・再読込時に古い購読を残さない。
...
4. transaction で `taskListOrder/{uid}` に追加し、同時に `taskLists/{taskListId}.memberCount` を +1
5. `/app` ページへ遷移
...
- production readiness 評価で挙がった認可モデル再設計（item1）は、2026-03 時点では対応不要と判断し、現行の共有権限モデルを維持します。
...
- **Update**: 自分の `taskListOrder` に登録されているか、または「有効な共有コード」を知っている場合に許可されます。不変フィールド（`id`、`createdAt`）の変更は防止されます。`memberCount` は通常不変で、認証済みの参加/離脱 transaction による ±1 のみ許可されます。
- **Delete**: 自分の `taskListOrder` に登録されており、かつ `resource.data.memberCount <= 1` の場合のみ許可されます（最後の保持者のみ実体削除可能）。

### 作成権限（Create）

認証済みユーザーであれば誰でもタスクリストを作成できます：

- 必須フィールド（`id`、`name`、`tasks`、`history`、`shareCode`、`background`、`memberCount`、`createdAt`、`updatedAt`）の存在確認
...
タスクリスト実体の削除は、最後の保持者（`memberCount <= 1`）のみ可能です：

- 通常の削除操作は `taskListOrder` からの離脱（remove）として扱います
- 離脱後に保持者が 0 になる場合のみ `taskLists` 実体と `shareCodes` を削除します
