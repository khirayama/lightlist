...
- フォーカスの引き継ぎ判定はページ切替開始時点の追加入力欄の実フォーカス状態を基準にし、過去にフォーカスしていた状態を後続のページ切替へ持ち越さない。
...
- 編集の保存はタスクリスト詳細の本文編集と同じ入力 parser と `history` 更新を使い、シートで指定した `date` / `pinned` は明示値として適用する。追加先タスクリストを変更した場合はタスクをリスト間移動する（元リストの `tasks.<id>` を削除し、移動先へ `taskInsertPosition` / `autoSort` に従って挿入、`history` は移動先へ追加。1 つの batch write で行い、元リストの mutation queue で直列化する）。失敗時は画面内にエラーを表示する。
