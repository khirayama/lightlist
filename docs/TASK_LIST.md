...
- **タスク詳細:** 選択中リストのタスクを `TaskListPanel` に渡して表示。完了・削除・追加・編集をまとめて受け持ち、履歴候補 (`history`) を `datalist` で提示する。セクション全体に `TaskList.background` を適用し、タスクエリアがリストの色で覆われる。
...
- **背景:** 選択中のタスクリストに設定された `background` をセクション全体に反映
- 並び替えハンドルの右に、タスクリストの背景色を示す角丸ボックスを配置
