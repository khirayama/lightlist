...
- Cloudflare Pages の Web build は Node.js `24.18.0` を前提とし、`apps/web/.node-version` で固定する。Pages の Root directory は `apps/web`、output directory は `dist` とし、`apps/web/package.json` の `packageManager` は `npm@12.0.1` に固定する。Node.js 22.22.2 同梱 npm から npm 12 への直接更新は `promise-retry` 欠損で失敗するため使用しない。
