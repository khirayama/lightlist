...
- Web は外部スタイル生成ライブラリを使わず、`apps/web/src/styles/globals.css` の通常 CSS と `apps/web/src/styles/compiled-styles.css` でスタイルを保持する。モーションは `globals.css` の `ll-anim-*` / `ll-pressable` 等の named class で管理し、3 プラットフォームとも OS / ブラウザの reduce motion 設定を尊重する。タスク行は追加約 240ms、削除約 120ms、並び替え・autoSort配置変更約 220ms相当、ドラッグ中 opacity `0.8` + scale `1.03` を共通仕様とし、削除はfade完了後に表示と保存対象から外す。WebのautoSort配置変更は同一配置へのlistener再同期でアニメーションを再生成しない。フォント CSS は bundle に巻き込まず、各 HTML entry で public asset として非 render-blocking（preload + `noscript` fallback）で読む。新規スタイルは通常 CSS または既存の named class を優先する。
...
- 設定・カレンダー画面は Android のパレット割当（`surfaceDim` / `surfaceContainer` / `outlineVariant` / `onSurfaceVariant`）を 3 プラットフォーム共通の正とし、淡いページ背景 + **ボーダーレス**の面カード（radius 12・枠線/影なし）+ カード内見出し（muted）で構成する。iOS は `AppPalette`（dynamic color）で表現し system grouped background は使わない。設定行の操作領域は 44pt / 48dp 以上。セクション順は「アカウント → 表示と動作 → 法的情報 → アカウント操作」、autoSort はタイトル + 説明の 2 行 + スイッチ、ログアウト/アカウント削除はボタンでなくテキスト行（削除は error 色）で 3 プラットフォーム統一（詳細は `AGENTS.md`）。
- カレンダー画面は淡いページ背景にグリッドを直置きし、日付付きタスク一覧のみボーダーレスの面カードに入れる。月ナビは枠なしアイコンボタン、日セルは選択 = 塗り円 / 当日 = 枠線円、日付ドットに縁取りなし。タスク行は「日付 + リストチップのメタ行 + 本文」の 2 行構成で、日付/本文タップ = 日付選択（絞り込みなし・ハイライト + スクロール）、チップタップ = タスクリストを開く、を 3 プラットフォームで統一する。日付選択円とタスク行ハイライトは短い共通モーションで切り替え、Reduce Motion 時は即時反映する（詳細は `AGENTS.md`）。
...
- task action は「右寄せの閉じる操作 → ボーダーレスのピン留め行 → muted の日付クリア → ボーダーレスの月カレンダー」の順とし、面は radius 12 の gray-50 / gray-950 相当で 3 プラットフォームを揃える。
...
- iOS / Android の compact 幅タスクリスト詳細は、戻るボタン行とページャーインジケータ行を分離し、入力欄の追加ボタンは入力文字がある時だけ表示する。未完了トグルは薄い枠線円、完了トグルは薄いグレー塗り円で描画し、参考画面に近い密度へ寄せる。完了トグルの塗り円（チェックマーク無し）と task 本文 weight（通常/完了 = SemiBold、`pinned && !completed` = Bold）は Web を含む 3 platform で統一する。タスク追加入力欄（角丸 14 / padding 14×10 / 最小高 44 / 背景不透明度 0.92 / gray-300 相当の枠線）、完了 task 行の不透明度 0.55、未完了トグル円 20（塗りなし）、インジケータドット 8（非選択 40%）、認証・共有カード角丸 24 + 枠線も 3 platform 共通とする（詳細は `AGENTS.md`）。
...
- iOS / Android の `TaskListDetailPage` は、タスク追加・完了切替・本文編集・日付変更・ピン切替ごとに task 集合の正規化と local pending 反映を一度だけ行い、その同じ集合から Firestore の変更前後の差分 field を保存する。`autoSort` 有効時も同じ順序で再採番したうえで、変化した `order` だけを書き込む。
...
- iOS の起動は `LightlistApp.init` で Firestore の無制限 `PersistentCacheSettings` 設定と `warmUpStartupData()`（`settings` / `taskListOrder` / `taskLists` chunk の `source: .cache` 先読み）を行い、言語・テーマ（`lightlist.language` / `lightlist.theme`）と `taskListOrder` の ID 列（`lightlist.taskListOrder.<uid>`）を UserDefaults にキャッシュして Firestore snapshot 到着を待たずに初回描画・chunk 購読を開始する。
- Android の起動は `MainActivity.onCreate` の `warmUpStartupData()` で locales パースと Firestore cache 先読み（`settings` / `taskListOrder` / `taskLists` chunk を `Source.CACHE`）を background thread で先行し、ログイン済みの `TaskList/__initial__` 自動遷移はアニメーションなしで行う。release は `androidx.profileinstaller` を依存に含める（詳細は `AGENTS.md`）。
...
- Web の Firestore IndexedDB cache に `settings` / `taskListOrder` / `taskLists` の実データがある場合は、placeholder / skeleton より cache hydrate 済み実データ表示を優先する。Web は auth 復元を待たず、localStorage `lightlist.lastUid` を `activeUid` として購読を先行開始し cache データを即描画する（auth-free 以外のページは mount 前に `getAuthInstance()` / `getDbInstance()` と `warmUpStartupData()` を実行し、persistent cache のコールド初期化と `settings` / `taskListOrder` / `taskLists` chunk の cache 先読みを React 初回描画と並行開始する。taskListOrder の順序付き ID 列は localStorage `lightlist.taskListOrder.<uid>` にキャッシュし、Web の persistent cache は `cacheSizeBytes: CACHE_SIZE_UNLIMITED` で LRU GC を無効化する）。Web / iOS / Android の `taskLists` chunk は cache snapshot / live snapshot ともに `docChanges()` ではなく snapshot 全体を chunk 単位で反映し、chunk 内の既存保持分を一度外してから snapshot documents で再構築する。
