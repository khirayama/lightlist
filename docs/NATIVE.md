...
- `expo-splash-screen`: 起動時のネイティブスプラッシュ制御（認証状態確定 + ルートレイアウト完了まで保持）
- `react-native-screens`: Expo Go に合わせたバージョン固定でネイティブスタックを安定化
- 依存解決: React は `apps/native/package.json` で管理し、ルート `package.json` の `overrides` は Expo Go 互換性が必要な依存（例: `react-native-screens`）に限定する
- `@lightlist/sdk`: Firebase 認証の呼び出しは SDK 経由
- React: `apps/native/package.json` で管理し、Expo Go の renderer と整合するバージョンを採用する
...
- 翻訳リソース: `apps/native/src/locales/*.json`（`ja` / `en` / `es` / `de` / `fr` / `ko` / `zh-CN` / `hi` / `ar` / `pt-BR` / `id`）
...
- アプリアイコン/スプラッシュ画像は `apps/native/assets`（`icon.png` / `adaptive-icon.png` / `splash-icon.png`）を正本とし、必要に応じて `ios/LightlistDev/Images.xcassets` と `android/app/src/main/res` の生成済み画像を同じロゴで更新する。現行アプリアイコン（`icon.png` / `adaptive-icon.png`）は既存デザインを踏襲し、ロゴレイヤーのみを直前版から `85%`（当初比 `76.5%`）に縮小して上下余白を増やした配置を採用する
...
- `expo-splash-screen` を使用し、認証状態の確定とルートレイアウト完了の両方を満たすまでネイティブスプラッシュを保持する。フォント読み込みはバックグラウンドで継続し、初回表示を優先する
- `App.tsx` は `StartupSplash` を起動時待機 UI と `Suspense fallback` の共通表示として利用し、遅延ロード中の空白表示を防ぐ
- 言語切替で LTR/RTL が変わる場合は、`settings.language` から算出した `uiDirection` を使ってナビゲーション方向・Drawer位置・アイコン向きを即時反映する（再起動不要）
- 方向判定は `I18nManager` ではなく `uiDirection` を単一の基準として使い、Expo Go / Development Build / 本番で同一挙動にする
- `AppDirectionContext`（`useAppDirection`）を経由して方向を各コンポーネントへ配布し、`AppIcon`（arrow-back）や `Drawer`、`Carousel` の RTL/LTR を統一する
- `Carousel` は `PagerView` に `layoutDirection={uiDirection}` を渡し、方向変更時は `key` の更新でページャーを再マウントしたうえで `setPageWithoutAnimation(currentIndex)` を実行し、表示ページと外部 index のズレを防ぐ
...
- `ShareCodeScreen` の共有リスト購読は share code 読み込み `useEffect` の cleanup で解除し、画面内の購読解除管理を単一路線にしている
...
- `password-reset?oobCode=...` のディープリンクを `PasswordResetScreen` にマッピングし、パスワード再設定を実行する。scheme は `APP_ENV` に応じて `lightlist` / `lightlist-staging` / `lightlist-dev` を使用する
...
- ドロワー内ヘッダー直下に「カレンダーで確認」ボタンを配置し、モーダルシートで未完了かつ日付付きタスクを月ごとに確認できる。カレンダー領域は `Carousel` で月単位に横スライドし、上部インジケーター操作とスワイプで移動する。`react-native-calendars` は Web 側と同じトーンになるよう日セル（36px角・選択時反転配色・today枠線）と日付ドット（最大3件）をカスタマイズし、日付選択で同日タスクへスクロール、タスクリスト名タップで対象リストへ遷移できる。表示中の月はインデックスではなく `YYYY-MM` の月キーで保持し、再計算時の月ジャンプを防ぐ。共通 `Carousel` は `react-native-pager-view` ベースの Controlled API（`index` / `onIndexChange`）で index を管理し、`onPageSelected` で確定する。外部からの index 変更時はページャーへ直接反映して連続スワイプ時の逆戻りを防ぐ。
...
- タスクリスト詳細はカルーセルで横スワイプ切り替えでき、スワイプ位置と選択中のリストIDを同期する。通常のスワイプ操作はカルーセルを優先し、`Carousel` は `index` / `onIndexChange` の Controlled 制御を前提とする。`scrollEnabled` は `TaskListCard` から受け取る `onSortingChange` と連動して、タスク並び替え中のみ横スワイプを無効化する。上部インジケーター（ドット）のタップでも切り替え可能。モバイルのドロワーからタスクリストを選択する場合は `InteractionManager.runAfterInteractions` でドロワー閉鎖後に選択状態を反映し、カルーセル位置との同期ずれを防止する
...
- 設定画面は Web と同じ情報設計（ヘッダー / 表示と動作 / アカウント操作）で構成し、「表示と動作」内でテーマ/言語/追加位置/自動並び替えを更新し、アカウント操作でメール表示・ログアウト/アカウント削除に対応
- サインアウトはドロワーと設定画面から実行
- 画面文言は `apps/web/src/locales` のキー体系（`auth` / `common` / `taskList` / `pages.*`）を基準にし、必要な設定キー（`settings.actions.*` / `settings.signingOut` など）は Web / Native で共通化して i18next で管理する
- `GestureHandlerRootView` でアプリルートをラップし、ドラッグ操作を安定化
- 主要画面タイトル（`AuthScreen` / `PasswordResetScreen` / `SettingsScreen` / `ShareCodeScreen`）の主見出し `Text` には `accessibilityRole="header"` を付与し、VoiceOver / TalkBack の見出し移動で到達できるようにする
- 設定画面の言語 / テーマ / 追加位置は select 行タップでダイアログを開く方式にし、行ではラベル + 現在値 + 矢印アイコンを表示、ダイアログではチェック付き選択肢リストで値を更新する
- タスク完了のカスタム切り替えは `accessibilityRole="checkbox"` と `accessibilityState.checked` を使用し、完了/未完了状態を読み上げ可能にする
- `components/ui/Dialog` の背景スクラムは既定で読み上げ対象外とし、ダイアログ本文の理解を優先する
...
- パスワードリセット用に `EXPO_PUBLIC_PASSWORD_RESET_URL` を必須で設定する（本番では `localhost` を使用しない）
- `@lightlist/sdk/firebase` は package `exports` の `react-native` 条件で `packages/sdk/src/firebase/index.native.ts` が選択され、上記 `EXPO_PUBLIC_FIREBASE_*` を参照する。
...
## アクセシビリティ確認（VoiceOver / TalkBack）

- iOS（VoiceOver）/ Android（TalkBack）の両方で、`AuthScreen` / `AppScreen` / `SettingsScreen` / `ShareCodeScreen` / `PasswordResetScreen` を確認する
- 確認項目:
  - 画面タイトルが見出しとして読み上げられる
  - 設定画面のカスタムラジオがラジオとして読み上げられ、選択状態が伝わる
  - タスク完了操作がチェックボックスとして読み上げられ、状態変化が伝わる
  - ダイアログ表示時に背景スクラムが不要な読み上げ対象として出現しない
  - カルーセルのインジケーターがボタンとして読み上げられ、現在位置が分かる
...
- `apps/native/src/App.tsx`: 起動時のスプラッシュ解除条件を「認証状態の確定 + ルートレイアウト完了」にし、`AppScreen` / `SettingsScreen` / `ShareCodeScreen` / `PasswordResetScreen` の遅延ロード中も `StartupSplash` を表示
...
- `apps/native/metro.config.js`: NativeWind 用 Metro 設定に加えて、monorepo 環境の `nodeModulesPaths` / `disableHierarchicalLookup` / `extraNodeModules` を設定し、`react-native` の二重解決を防止
...
- `apps/native/src/components/ui/StartupSplash.tsx`: 起動時および遅延ロード待機時に表示する全画面ローディングUI
...
- `apps/native/src/locales/*.json`: 11言語の翻訳リソース
- `apps/native/src/styles/theme.ts`: テーマ定義とリストカラー
- `apps/native/app.config.ts`: Expo 設定。`expo-splash-screen` プラグインで light/dark のスプラッシュ背景色と画像を設定し、`plugins` に `expo-font` と `@react-native-community/datetimepicker` を設定する
- `apps/native/app.json`: `userInterfaceStyle` を `automatic` に変更し、テーマ切替に追従。`scheme` は開発用の `lightlist-dev` を保持し、実際の配布ビルドでは `app.config.ts` の `APP_ENV` に応じて `lightlist` / `lightlist-staging` / `lightlist-dev` を適用する。`orientation` を `default` にして回転に対応
- `apps/native/package.json`: `react-native-screens` を Expo Go に合わせて固定。開発用に `dev:lan` / `dev:local` / `start:lan` / `start:local` を追加。`react-native@0.81.x` の Metro 解決で `@react-native/virtualized-lists` が見つからない環境向けに `@react-native/virtualized-lists@^0.81.5` を明示依存として追加
- `apps/native/tsconfig.json`: `nativewind/types` を明示的に読み込み、`react-native` / `@react-native/virtualized-lists` の型解決先を `apps/native/node_modules` に固定して `className` 型拡張を安定化
- `package.json`: `react` は Expo SDK 54 / Expo Go に合わせて 19.1.0 に固定し、`apps/native/package.json` の `@types/react` は Expo 互換の `~19.1.10` を使用する。`react-native-screens` は Expo Go 互換で固定
