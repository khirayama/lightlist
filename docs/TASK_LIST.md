...
- task / taskList の並び替えはドラッグ操作に加えてスクリーンリーダー向け代替手段を提供する。Web はキーボード（dnd-kit `KeyboardSensor`）+ ローカライズ済み読み上げアナウンス、iOS は VoiceOver の `accessibilityAction`、Android は TalkBack の custom action として「上へ移動 / 下へ移動」（`a11y.moveUp` / `a11y.moveDown`）を提供する。
- ページャーインジケータのドットは「タスクリスト名 + 位置（`a11y.listPosition`）」を読み上げラベルとし、選択中ドットは選択状態を公開する。カレンダーの日セルは「日番号 + 今日（`a11y.today`）+ タスク有無（`a11y.hasTasks`）+ 選択状態」を読み上げる。
...
- iOS / Android の compact 幅タスクリスト詳細は、戻るボタン行とページャーインジケータ行を縦に分け、タイトル行・入力欄・操作列・タスク行までの余白を詰めて単票リファレンスに近い密度で配置する。入力欄の追加ボタンは入力文字がある時だけ表示し、未完了トグルは薄い枠線円、完了トグルは薄いグレー塗り円で表現する。完了トグルの塗り・行の透明度・ピン/カレンダーアイコンの切替・追加ボタンの表示切替は 3 プラットフォームともアニメーションし、OS / ブラウザの reduce motion 設定で無効化する。画面右端の action 群（ヘッダー共有・操作列のゴミ箱・task row のカレンダー/ピン）は個別 offset ではなく共通の列幅でアイコン中心線を 1 本に揃え、drag handle は並び替えなどテキスト横アイコンと目視サイズを揃える。
...
- Android のタスク入力欄の送信アイコンは iOS と同じく入力文字がある時だけ表示し、表示切替は横方向アニメーション（reduce motion 時は即時）で行う。Web は入力欄 focus 中に表示する。
...
- `taskLists` / `taskListOrder` / `shareCodes` の `createdAt` / `updatedAt` は Firestore Rules の型検証と pending snapshot の安定性に合わせ、server timestamp ではなく Unix epoch milliseconds の number として書き込む。
