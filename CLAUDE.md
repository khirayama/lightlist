...
- 設定画面は Web の淡いページ背景 + 境界線付きカード + カード内見出しを正とする。iOS は grouped semantic colors、Android は `surfaceDim` / `surfaceContainer` / `outlineVariant` で揃え、設定行の操作領域は 44pt / 48dp 以上を維持する。
