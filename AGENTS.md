...
- 設定画面の視覚階層は Web を正とし、淡いページ背景の上に境界線付きカードを置き、各セクション見出しはカード内先頭へ配置する。iOS は system grouped background / secondary grouped background、Android は `surfaceDim` / `surfaceContainer` と `outlineVariant` で同じ階層を表現し、設定行の 44pt / 48dp 以上の操作領域を維持する。
