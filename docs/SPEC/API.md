...
- `POST /api/auth/login` - ログイン（deviceId必須）
...
### 設定系
- `GET /api/settings` - ユーザー設定取得（theme, language）
- `PUT /api/settings` - ユーザー設定更新
- `GET /api/app` - App設定取得（taskInsertPosition, autoSort）
- `PUT /api/app` - App設定更新

### タスクリスト系
- `GET    /api/tasklists` - タスクリスト一覧取得
- `PUT    /api/tasklists/:taskListId/session` - タスクリストセッション開始
- `PATCH  /api/tasklists/:taskListId/session` - タスクリストセッション更新
- `DELETE /api/tasklists/:taskListId/session` - タスクリストセッション終了
- `GET    /api/tasklists?token=token` - 共有タスクリスト閲覧（読み取り専用）
- `POST   /api/tasklists/:taskListId/token` - タスクリスト共有トークンを生成・更新


### 注意事項

- タスクリスト・タスクの作成・更新・削除・取得は/api/tasklists/:taskListId/session経由で行う
